...
for strain in strain_info:
	...
	try:
		...
		for resistant in [i+"_R" for i in antibiotics]:
	...
	try:
		...
		for susceptable in [i+"_S" for i in antibiotics]:
			sub_dict[susceptable] = 0
		susfail = True

	if(not resfail):
		for resistant in antibiotics:
			if(resistant in resistant_antibiotics):
				sub_dict[resistant+"_R"] = 1
			else:
				sub_dict[resistant+"_R"] = 0
	if(not susfail):
